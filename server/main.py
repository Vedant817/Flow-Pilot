...
def get_service():
    """Get an authorized Google Sheets API service."""
    credentials = service_account.Credentials.from_service_account_file(
        'service-account.json', scopes=SCOPES)
    return build('sheets', 'v4', credentials=credentials)

def setup_watch():
    """Set up a watch on the Google Sheet."""
    service = build('drive', 'v3', credentials=get_credentials())
    
    # Set up the watch request
    body = {
        'id': 'your-channel-id',  # A unique ID for your channel
        'type': 'web_hook',
        'address': 'https://your-domain.com/webhook',  # Your webhook URL
...
def webhook():
    ...
    if 'X-Goog-Resource-State' in headers:
        state = headers['X-Goog-Resource-State']
        
        if state == 'change':
            # The file has changed, read the current content
            current_content = read_google_sheet()
            
            # Compare with previous content and process changes
